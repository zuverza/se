...
//		$option				= JRequest::getCmd( 'option' );
		...
		// Keyword filter
		if ($search) {
			$where[] = 'LOWER( c.address1 ) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false );
			$where[] = 'LOWER( c.address2 ) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false );
			$where[] = 'LOWER( co.country_name ) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false );
			$where[] = 'LOWER( s.state_name ) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false );
			$where[] = 'LOWER( z.zone_name ) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false );
		}
		...
		// Get the products
		$query = 'SELECT c.*, at.* ' .
				' FROM #__tienda_adresses AS c' .
				' LEFT JOIN #__tienda_addresstypes at ON pp.address_type_id = c.address_type_id '.
				' LEFT JOIN #__tienda_countries co ON co.country_id = c.country_id '.
				' LEFT JOIN #__tienda_states s ON s.state_id = c.state_id '.
				' LEFT JOIN #__tienda_zones z ON z.zone_id = c.zone_id '.
				$where .
				$order;
