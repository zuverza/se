...
if(isset($uid) && isset($nid)){
	...
	//SELECT m. * , fu . *, fm.* FROM mash_up m,  file_usage fu, file_managed  fm WHERE m.mid  = 7 AND m.nid = fu.id AND fu.fid = fm.fid  

/*
SELECT m. * , fu. * , fm . *
FROM mash_up m, file_usage fu, file_managed fm
WHERE m.mid =7
AND m.nid = fu.id
AND fu.fid = fm.fid
LIMIT 0 , 30
...
////////////////////////////////////here///////////////////////////////////////////////////////
	...
	while($files = mysql_fetch_array($result)){
		...
		echo '<br/>../../../../default/files/'.$files['filename'];
	...
	//$files_to_zip = array('mash_up.module');
		...
		$result = create_zip($files_to_zip,'my-archive1.zip');
